...
class HelpPythonDialog : BaseDialogFragment() {
    ...
    private fun getColoredCode(s: SpannableString): SpannableString {
        val pattern = Pattern.compile("${PseudoKotlinView.RESERVED_LOOP}|" +
                "${AbsPseudoView.RESERVED_CONDITION_IF}|${AbsPseudoView.RESERVED_CONDITION_ELSE}")
        ...
        while (matcher2.find()) {
            s.setSpan(ForegroundColorSpan(ContextCompat.getColor(context!!, R.color.backgroundBlueLight)),
                    matcher2.start(), matcher2.end(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
        }
        return s
    }
